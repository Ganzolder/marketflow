...
export async function copyActivity(prevState: DeleteFormState | null, formData: FormData): Promise<DeleteFormState> {
    const validatedFields = DeleteActivitySchema.safeParse({
        campaignId: formData.get('campaignId'),
        actionId: formData.get('actionId'),
        activityId: formData.get('activityId'),
    ...
    try {
        await copyActivityData(campaignId, actionId, activityId);
    } catch (e) {
        const errorMessage = e instanceof Error ? e.message : "Произошла неизвестная ошибка.";
        return { message: `Ошибка базы данных: не удалось скопировать активность. ${errorMessage}`, error: true };
    }

    revalidatePath(`/campaigns/${campaignId}/${actionId}`);
    return { message: "Активность успешно скопирована." };
}
...
export type MechanicsFormState = {
  message: string;
  error?: boolean;
  errors?: z.ZodError<z.infer<typeof MechanicsSchema>>['formErrors']['fieldErrors']
};

export async function updateActionMechanics(prevState: MechanicsFormState, formData: FormData): Promise<MechanicsFormState> {
    const validatedFields = MechanicsSchema.safeParse({
        campaignId: formData.get('campaignId'),
        actionId: formData.get('actionId'),
    ...
    const { campaignId, actionId, mechanics } = validatedFields.data;

    try {
        await updateActionMechanicsData(campaignId, actionId, mechanics || '');
    } catch (e) {
        const errorMessage = e instanceof Error ? e.message : "Произошла неизвестная ошибка.";
        return { message: `Ошибка базы данных: ${errorMessage}`, error: true };
    }
    ...
    revalidatePath(`/campaigns/${campaignId}/${actionId}`);
    return { message: "Механика акции успешно обновлена." };
}
...
export async function updateTaskStatus(prevState: StatusFormState, formData: FormData): Promise<StatusFormState> {
    ...
    try {
        await updateTaskStatusData(validatedFields.data.taskId, validatedFields.data.status);
        revalidatePath('/tasks');
        revalidatePath('/');
...
const UpdateSalesKpiSchema = z.object({
  campaignId: z.string(),
  actionId: z.string(),
  salesKpiName: z.string(),
});

export async function updateActionSalesKpiName(formData: FormData): Promise<{message: string, error?: boolean}> {
    const validatedFields = UpdateSalesKpiSchema.safeParse({
        campaignId: formData.get('campaignId'),
        actionId: formData.get('actionId'),
        salesKpiName: formData.get('salesKpiName'),
    });

    if (!validatedFields.success) {
        return { message: "Ошибка валидации.", error: true };
    }
    
    const { campaignId, actionId, salesKpiName } = validatedFields.data;

    try {
        await updateActionSalesKpiNameData(campaignId, actionId, salesKpiName);
    } catch (e) {
        const errorMessage = e instanceof Error ? e.message : "Произошла неизвестная ошибка.";
        return { message: `Ошибка базы данных: ${errorMessage}`, error: true };
    }
...
export async function updateEridAction(erid: string): Promise<EridState> {
    if (typeof erid !== 'string') {
        return { message: "Неверный формат ERID.", error: true };
    }
    try {
        await updateErid(erid);
        return { message: 'ERID успешно обновлен.' };
    } catch (e) {
        const errorMessage = e instanceof Error ? e.message : "Произошла неизвестная ошибка.";
